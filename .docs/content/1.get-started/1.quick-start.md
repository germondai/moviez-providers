...
Let's get started with `@moviez/providers`. First lets install the package.

::alert{type="warning"}
**Note:** The `@moviez/providers` package is not published on npm. Install it directly from GitHub using the following commands:
::

::code-group
  ```bash [NPM]
  npm install @moviez/providers@github:moviez/providers#production
  ```
  ```bash [Yarn]
  yarn add @moviez/providers@github:moviez/providers#production
  ```
  ```bash [PNPM]
  pnpm add @moviez/providers@github:moviez/providers#production
  ```
::
...
```ts [index.ts (server)]
import { makeProviders, makeStandardFetcher, targets } from '@moviez/providers';
